...
5. **Hero.** Pinned for 2.2 viewport heights, scrubbed with 0.6s smoothing, reversible. At rest the navy field is empty: the type sits at opacity 0 and the render waits 40px below the bottom edge, so the only thing on screen is the scroll prompt.
   ...
   *In* (0–0.62): the render rises at 1.75× from the centre of the bottom edge and settles to the right of the type, large enough to overlap it — it covers the end of "New-Age", as the storyboard draws it. The type resolves alongside at a different rate. Each line is seven stacked copies clipped to horizontal bands, each translated up to 140px, sliding back into alignment; the outermost bands' clips bleed 40% past the line box so ascenders and the descender of "Age" stay whole.

   *Out* (0.72–1): the composition lifts up and away. The speaker rises out through the top, growing slightly so it reads as passing the camera rather than receding, while the type is taken away one band at a time from the bottom up — the letters keep their tops and are cut flat below, then vanish. The per-band fade (0.05) is deliberately short against the stagger (0.038) so only about one band is mid-fade at a time: that is what keeps the cut a hard edge travelling up the letterforms instead of the whole word going translucent at once. The block lifts as a whole; only opacity is staggered. Clip paths are static throughout, and only transform and opacity animate.
