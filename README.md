...
- **Navbar shape.** A floating rounded bar (max 1120px, 16px from the top) rather than an edge-to-edge strip, with the wordmark centred and three links each side. The pill is 84px wide holding only the mark. The bar/pill morph is a spring-eased `clip-path: inset(... round 28px)` on one glass element, so the backdrop layer never re-composites; links fade and go `visibility: hidden` so they can't take focus while collapsed. Hover, focus-within, an open dropdown or an upward scroll of 6px expand it.
