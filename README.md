...
- `src/sections/opening/` — the opening sequence: `useAssetProgress` (real font + hero-decode progress), `Opening.tsx` (loader, clear, navy fill, pinned scrub hero, indicator), `SlicedLine.tsx` (the type-resolve effect), `useHandoffResistance.ts` (the scroll-back gate).
...
6. **Handoff.** The pin releases and the navy scrolls off over the bone page. The navbar becomes visible at that moment but sits at a lower z-index than the hero, so the navy scrolling away literally reveals it — docked, rectangular, edge to edge. The stretch where the navy is leaving has no resting state: stop scrolling inside it and the page settles (0.8s, through Lenis so it never fights the smoothing) to the end it was heading for. Down settles at the **home top**, the point where the navy has fully gone and the bar's top edge meets the viewport top. Up settles back at the hero's final frame.

   Scrolling back up from the home page hits that home top and meets resistance through Lenis's `virtualScroll` hook: within 8px of it, upward wheel deltas are swallowed and the page nudges 14px to show a sliver of navy. A sustained gesture (900px cumulative within 160ms gaps) or two hard flicks (>70px each within 1.4s) pass; the snap then carries the page to the hero's final frame, and from there scrolling up plays the animation in reverse. The gate re-arms 24px back below the home top. Off entirely under `prefers-reduced-motion`.
...
- **Keyboard and touch are not gated.** The resistance only applies to wheel/trackpad input, which is what Lenis virtualises. Arrow keys, space, and native touch scroll straight through. "Never trap the user" seemed to outrank consistency.
...
- **Resistance feel.** The thresholds in `motion.ts` (`RESISTANCE_*`) were set by reasoning, not by hand on a trackpad. Expect to tune `RESISTANCE_SUSTAINED_PX` and `RESISTANCE_HARD_FLICK_PX` for Magic Trackpad vs. mouse wheel.
...
- **Handoff snap and resistance were tuned with synthetic wheel events**, not a hand on a trackpad. The snap idle (140ms) and duration (0.8s) live in `motion.ts`; if a real trackpad's momentum tail makes the settle feel late or eager, those are the two numbers.
