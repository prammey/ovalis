...
5. **Hero.** Pinned for 2.2 viewport heights, scrubbed with 0.6s smoothing, reversible. At rest the navy field is empty: the type sits at opacity 0 and the render waits 40px below the bottom edge, so the only thing on screen is the scroll prompt.
   ...
   *Out* (0.72–1): the type is taken away one band at a time from the bottom up — the letters keep their tops and are cut flat below, then vanish. The per-band fade (0.05) is deliberately short against the stagger (0.038) so only about one band is mid-fade at a time: that is what keeps the cut a hard edge travelling up the letterforms instead of the whole word going translucent at once. The block lifts a little as a whole; only opacity is staggered.
